...
public interface FriendTypeService extends BaseService<FriendType> {
	...
	//根据用户以及对应的分组得到其count数
	Integer getFriendCountByTypeId(int userId,int typeId);
	...
	FriendType getFriendTypeById(int id);

	void updateFriendType(@Param("typeId")int typeId, @Param("userId")int userId, @Param("friendId")int friendId);

	List<FriendType> getFriendTypeByUserName(int userId);

	List<FriendType> findFriendTypeByUserIdFixed(Integer userId);
}
