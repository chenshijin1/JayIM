        ...
        layui.each(cache.friend, function(index, item){
          //if(data.groupid == item.id){
            ...
            if(has) return layer.msg('好友 ['+ (data.username||'') +'] 已经存在列表中',{anim: 6});
              console.log("friend did not exist");
      ...
      /*console.log(JSON.stringify(cache));
        ...
        $.post('/type/getName', {
          ...
          console.log("res: " + JSON.stringify(res));
            groupName = res.typeName;
            console.log("groupName: " + groupName);
            for(var i in cache.friend){
                console.log("group name: " + cache.friend[i].groupname);
                if(cache.friend[i].groupname == groupName){
                    index = i;
                    break;
                }
            }
            console.log("index: " + index);
            li.find('.layim-count').html(cache.friend[index].list.length); //刷新好友数量
        });*/
